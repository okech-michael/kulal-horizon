      ...
      <div className="container-editorial relative py-20">
        <div className="grid gap-14 lg:grid-cols-12">
          <div className="lg:col-span-5">
            ...
            <p className="mt-6 max-w-md text-white/95 leading-relaxed">
            ...
            <form onSubmit={(e) => e.preventDefault()} className="mt-8 max-w-md">
              <label className="block text-xs uppercase tracking-[0.22em] text-white/95 font-semibold mb-3">Field notes newsletter</label>
              <div className="flex gap-2 rounded-full glass-dark p-1.5">
                <input type="email" placeholder="you@email.com" className="flex-1 bg-transparent px-4 py-2 text-sm text-white placeholder:text-white/70 outline-none" />
        ...
        <div className="mt-16 border-t border-white/10 pt-6 flex flex-wrap items-center justify-between gap-4 text-xs text-white/90">
...
function FooterCol({ title, links }: { title: string; links: string[] }) {
  return (
    <div>
      <div className="text-xs uppercase tracking-[0.22em] text-white/95 font-semibold">{title}</div>
      <ul className="mt-5 space-y-3">
        {links.map((l) => (
          <li key={l}><a href="#" className="text-sm text-white/90 hover:text-highlight transition-colors">{l}</a></li>
        ))}
      </ul>
    </div>
  );
